...
const ContractSupervisorPicker: FC<Props> = ({contract, setContract}) => {
  ...
  useEffect(() => {
    if (usersList) {
      const userOptions: Option[] = usersList
        .filter(user => user.role === UserRoles.Company)
        .map(user => ({
          user,
          value: user.id,
  ...
  return (
    <ReactSelect
      ...
      placeholder={t('contracts.select_supervisor')}
      isDisabled={!usersList || !!contract.id} // disable for contract update
      value={userOptions.find(option => option.user.id === contract.supervisorId)}
      onChange={option => setContract({...contract, supervisorId: option?.value ?? 0})}
    />
  );
};
